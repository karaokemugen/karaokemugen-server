...
export async function getAllKaras(filter, lang, from = 0, size = 99999999999) {
	try {
		const pl = await selectAllKaras(filter, lang);
		return formatKaraList(pl.rows.slice(from, from + size), lang, from, pl.rowCount);
	} catch(err) {
		throw err;
	}
}
