...
export async function webOptimize(source, destination) {
	try {
		return await execa(getConfig().Path.Bin.ffmpeg, ['-y', '-i', source, '-movflags', 'faststart', '-acodec' , 'copy', '-vcodec', 'copy', destination], {encoding: 'utf8'});
	} catch(err) {
...
export async function getMediaInfo(mediafile) {
	try {
		...
		if (indexDuration > -1) {
			...
			durationInt = timeToSeconds(duration);
		} else {
			error = true;
		}
