...
export async function getLatestGitCommit(): Promise<string> {
	const commit = await fs.readFile(resolve(getConfig().System.Repositories[0].BaseDir, '.git/refs/heads/master'), 'utf-8');
	return commit;
}
...
export async function getGitDiff(commit: string): Promise<string> {
	...
	const diff = await gitDiff(commit, latestCommit, resolve(getConfig().System.Repositories[0].BaseDir));
	return diff;
}

export async function initGitRepos() {
	gitConfig(getConfig().System.Repositories[0].BaseDir);
}