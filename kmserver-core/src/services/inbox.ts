...
import {deleteInbox, insertInbox, selectInbox, updateInboxDownloaded, updateInboxStatus, updateInboxUnassign} from '../dao/inbox.js';
...
export async function setInboxStatus(inid: string, status: InboxActions, reason?: string) {
	try {
		...
		if (status === 'changes_requested') {
			if (user.flag_contributor_emails) await sendMail(
				i18n.t('MAIL.INBOX.CHANGES_REQUESTED.SUBJECT', { lng: getUserLanguage(user) }),
			...
			if (inbox.gitlab_issue) {
				...
				await postNoteToIssue(issueNumber, repoName, `Waiting for changes by original uploader. Notes from reviewer:
				${reason}
				`);
			}
		} else if (status === 'in_review') {
			if (user.flag_contributor_emails) await sendMail(
				i18n.t('MAIL.INBOX.IN_REVIEW.SUBJECT', { lng: getUserLanguage(user) }),
			...
			if (inbox.gitlab_issue) {
				const issueNumber = getGitlabIssueNumber(inbox.gitlab_issue);
				await postNoteToIssue(issueNumber, repoName, `Upload is being reviewed by ${inbox.username_downloaded}`);
			}
		} else if (status === 'accepted') {
			if (user.flag_contributor_emails) await sendMail(
				i18n.t('MAIL.INBOX.ACCEPTED.SUBJECT', { lng: getUserLanguage(user) }),
			...
			if (user.flag_contributor_emails) await sendMail(
				i18n.t('MAIL.INBOX.REJECTED.SUBJECT', { lng: getUserLanguage(user) }),
			...
			if (inbox.gitlab_issue) {
				const issueNumber = getGitlabIssueNumber(inbox.gitlab_issue);
				await postNoteToIssue(issueNumber, repoName, `Upload was REJECTED by ${inbox.username_downloaded}: ${reason}`);
				await closeIssue(issueNumber, repoName);
			}
		}
	} catch (err) {
...
export async function addKaraInInbox(kara: KaraFileV4, contact: {name: string, login?: string}, issue?: string, edited_kid?: string, inid?: string) {
	try {
		inid = inid || uuidV4();
		await insertInbox({
			inid,
...
export async function removeKaraFromInbox(inid: string, authToken: JWTTokenWithRoles) {
	try {
		...
		const promises = [];
		promises.push(removeInboxFiles(inbox.edited_kid || inbox.kid, inbox.karafile, inbox.mediafile, inbox.lyrics_infos ? inbox.lyrics_infos[0]?.filename : null));
		promises.push(deleteInbox(inid));
		if (inbox.gitlab_issue) {
			const numberIssue = getGitlabIssueNumber(inbox.gitlab_issue);
			const repoName = getConfig().System.Repositories[0].Name;
			if (inbox.username === authToken.username) promises.push(postNoteToIssue(numberIssue, repoName, 'Closing by request of original uploader'));
...
export async function clearProcessedInboxes(karas: KaraFileV4[]) {
	...
	const kids = new Set(karas
		.filter(k => k.data.repository !== 'Staging')
		.map(k => k.data.kid));
