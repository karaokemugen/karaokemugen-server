...
export async function getPlaylists(params: PLParams, token: JWTTokenWithRoles): Promise<DBPL[]> {
	try {
		...
		return pls.filter(pl =>
			...
			token?.roles.admin ||
			pl.username === token?.username.toLowerCase() ||
			pl.contributors.find(c => token?.username.toLowerCase() === c.username));
	} catch (err) {
...
export async function importPlaylist(playlist: PlaylistExport, token: JWTTokenWithRoles) {
	...
	try {
		...
		const users = new Map();
		if (!playlist.PlaylistInformation.contributors) playlist.PlaylistInformation.contributors = [];
