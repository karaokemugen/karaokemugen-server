	...
	render() {
		...
		tagList = tagList.map(function(tag){
			return {
				...
				link : "/karas?"+querystring.stringify(filterTools.clear().addTag('songtype',tag.tid,tag.slug).getQuery()),
			...
			<div>
				...
				<div className="kmx-filter-line">
					<Pagination
						...
						size={pageSize}
						current={page}
						renderUrl={(i) => { return "/songtypes?"+querystring.stringify(filterTools.reset().setPage(i).getQuery()); }}
						/>
				...
				<DedicatedTagtList type="songtypes" tags={tagList} pageSize={pageSize} page={page} orderBy={this.state.orderBy}/>
