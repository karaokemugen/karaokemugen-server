...
class Page extends React.Component {
	...
	render() {
		...
		for (let id in this.props.tags) {
			...
			if(tag.types.includes(tagsMap.misc.id))
			{
				kmax = Math.max(kmax,tag.karacount[tagsMap.misc.id]);
		...
		tagList = tagList.map(function(tag){
			return {
				...
				karacount : tag.karacount[tagsMap.misc.id],
				link : filterTools.clear().addTag('misc',tag.tid,tag.slug).getQuery().url,
				height : 100 * tag.karacount[tagsMap.misc.id] / kmax
			};
		})
		...
		return (
			<div>
				<Head>
					<title key="title">{i18n.t('sitename')} - {i18n.t('category.miscs')}</title>
				</Head>
				...
				<DedicatedTagtList type="miscs" tags={tagList} pageSize={pageSize} page={page} orderBy={this.state.orderBy}/>
