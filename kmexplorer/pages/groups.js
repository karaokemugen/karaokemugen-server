...
class Page extends React.Component {
	...
	render() {
		...
		for (let id in this.props.tags) {
			...
			{
				kmax = Math.max(kmax,tag.karacount[tagsMap.group.id]);
		...
		tagList = tagList.map(function(tag){
			return {
				...
				karacount : tag.karacount[tagsMap.group.id],
				link : filterTools.clear().addTag('group',tag.tid,tag.slug).getQuery().url,
				height : 100 * tag.karacount[tagsMap.group.id] / kmax
			};
		})
			...
			<div>
				...
				<div className="kmx-filter-line">
					<Pagination
						...
						size={pageSize}
						current={page}
						renderUrl={(i) => { return filterTools.reset().setPage(i).getQuery('groups').url; }}
						/>
