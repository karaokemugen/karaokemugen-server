...
class Page extends React.Component {
	...
	render() {
		...
		for (let id in this.props.tags) {
			...
			if(tag.types.includes(tagsMap.author.id))
			{
				kmax = Math.max(kmax,tag.karacount[tagsMap.author.id]);
			...
			<div>
				...
				<div className="kmx-filter-line">
					<Pagination
						...
						size={pageSize}
						current={page}
						renderUrl={(i) => { return filterTools.reset().setPage(i).getQuery('authors').url; }}
						/>
				...
				<DedicatedTagtList type="authors" tags={tagList} pageSize={pageSize} page={page} orderBy={this.state.orderBy}/>
