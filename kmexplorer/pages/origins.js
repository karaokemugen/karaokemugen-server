...
class Page extends React.Component {
	...
	render() {
		...
		for (let id in this.props.tags) {
			...
			if(tag.types.includes(tagsMap.origin.id))
			{
				kmax = Math.max(kmax,tag.karacount[tagsMap.origin.id]);
		...
		tagList = tagList.map(function(tag){
			return {
				...
				karacount : tag.karacount[tagsMap.origin.id],
				link : filterTools.clear().addTag('origin',tag.tid,tag.slug).getQuery().url,
				height : 100 * tag.karacount[tagsMap.origin.id] / kmax
			};
		})
			...
			<div>
				...
				<div className="kmx-filter-line">
					<Pagination
						...
						size={pageSize}
						current={page}
						renderUrl={(i) => { return filterTools.reset().setPage(i).getQuery('origins').url; }}
						/>
