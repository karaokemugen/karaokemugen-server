<template>
	<nav class="pagination" role="navigation" aria-label="pagination">
		...
		<ul class="pagination-list">
			...
			<li v-if="page > 1">
				<a class="pagination-link" :aria-label="$t('search.aria.goto', [page - 1])"
				   @click="changePage(page - 1)">{{ page-1 }}</a>
			</li>
			<li>
			...
			<li v-if="page <= (lastPage - 1)">
				<a class="pagination-link" :aria-label="$t('search.aria.goto', [lastPage])"
				   @click="changePage(lastPage)">{{ lastPage }}</a>
			</li>
		</ul>
	</nav>
</template>
